...
class TestPokemons:
    ...
    def test_post_pokemons(self, auth_session):
        response_post = auth_session.post(f"{self.base_url}/pokemons", json = self.json_data)
    #  Проверка допустимых статус-кодов ответа (201, 422)
        assert response_post.status_code in (201, 422), f"Неожиданный статус {response_post.status_code}"
        if response_post.status_code == 201:
            pokemons_post_body = response_post.json()
            self.pokemon_id = response_post.json()["id"]                    ## Сохранения id в переменную покемона для класса
            assert "id" in pokemons_post_body, "Покемон не создан"          ## Проверили что id есть в теле ответа Post
            print(f"Создан покемон {self.pokemon_id}")
        else:
            error_body = response_post.json()
            assert "status" in error_body, "Параметр 'status' отсутствует в ответе на создание покемона"
            assert "message" in error_body, "Параметр 'message' отсутствует в ответе на создание покемона"
            print(error_body)
    #Удаление покемона после созадние
        delete_pokemons = auth_session.post(f"{self.base_url}/pokemons/knockout", json={"pokemon_id": self.pokemon_id})
        assert delete_pokemons.status_code == 200
        print(f'Покемон {self.pokemon_id} удален после создания запроса POST')

    def test_path_pokemons(self, auth_session, patch_body):
        response_post = auth_session.post(f"{self.base_url}/pokemons", json=self.json_data)
    ...
    # Изменение покемона
        ...
        print(f'ОТВЕТ PUT: ID покемона полученный из запроса PUT{response_pokemon} = ID покемона при создании POST{pokemon_id}')
    # Удаление покемона
        response_delete = auth_session.post(f'{self.base_url}/pokemons/knockout', json={"pokemon_id": pokemon_id})
        assert response_delete.status_code == 200
        print(f"ОТВЕТ DELETE(POST): Тело ответа на удаление покемона {response_delete.json()}")
        assert response_delete.json()["message"] == f"Покемон в нокауте" and response_delete.json()["id"] == f"{pokemon_id}"
